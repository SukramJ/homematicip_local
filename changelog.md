...
- The openccu-loom backend no longer needs a source-code master switch to become available. The compile-time constant `LOOM_BACKEND_SELECTABLE` is gone; the config flow now surfaces the backend exactly when it is relevant. A daemon announcing itself via mDNS (`_openccu-loom._tcp`) produces a discovery card as usual, and the backend menu in the manual setup appears when a loom entry is already configured or a daemon discovery flow is in progress. Setups without a daemon are unaffected and go straight to the direct-CCU step as before. The now-unreachable `loom_not_enabled` abort message was removed from `strings.json` and the en/de translations
- The openccu-loom backend is marked **Beta** at every entry point, so its maturity is visible before a user commits to it: the discovery card and the reauth/reconfigure dialogs show `openccu-loom Beta` via the `{backend}` flow-title placeholder, and the backend menu entry, the manual `loom` step and the discovered-daemon `loom_token` step carry a `(Beta)` title plus a leading note that the direct CCU connection remains the recommended setup. The options flow of an already configured loom entry is deliberately left unmarked
...
- Config flow and diagnostics groundwork for the openccu-loom backend (backend-aware flow titles, in-place backend switch on a matching CCU serial, sub-device toggle, serial-keyed manual setup, diagnostics on the compat adapter)
